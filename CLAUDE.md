...
**The MVP (Fase 1a–1e) is feature-complete on the backend.** Still pending: user validates web/app builds (`npm install` for echarts; `flutter analyze/test`); wiring real push (FCM/APNs) + email digest behind the dispatcher seam; Fase 2 (public lead-capture form) and the Futuro items (WhatsApp, documents, email nurture).
