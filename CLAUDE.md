...
Remaining Column B: real push (FCM/APNs) + email sender beans behind the seams; export CSV/PDF (RF-10); dashboard period-filter UI (backend already takes from/to); swap hand-written client services for generated OpenAPI clients; Flutter secure token storage; bulk lead reassignment on user deactivation (RN-02). Then Column C (Fase 2 public capture; Futuro: WhatsApp/documents/nurture/TEAM_LEAD) and Column D (deploy/Dockerfile/CI-prod, front + E2E tests, observability).
